...
def decrypt(encryption: str) -> str:
    chars_lst = list(encryption)
    idx = 0
    while idx <= len(chars_lst):
        if idx < len(chars_lst) - 1:
            curr_sym = chars_lst[idx]
            next_sym = chars_lst[idx + 1]
            if curr_sym == '.' and next_sym == '.':
                del chars_lst[idx - 1: idx + 2]
                if idx >= 2:
                    idx -= 2
                else:
                    idx = 0
            elif curr_sym == '.' and next_sym != '.':
                del chars_lst[idx]
                idx -= 2
        elif idx == len(chars_lst) - 1 and chars_lst[idx] == '.':
            print(f"Удаляем [{chars_lst[idx]}]", end=" ")
            del chars_lst[idx]
        idx += 1
    result = "".join(chars_lst) if len(chars_lst) > 0 else "empty string"
    return result
